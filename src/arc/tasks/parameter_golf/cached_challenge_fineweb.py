    ...
    if not path.is_file():
        if skip_manifest_download:
            raise FileNotFoundError(
                "manifest.json is required for manifest-driven shard counts "
                f"but is not present locally at {path}"
            )
...
def main() -> None:
    ...
    dataset_entry = next(
        (item for item in manifest.get("datasets", []) if item.get("name") == dataset_dir),
        None,
    )
    ...
    tokenizer_entry = next(
        (item for item in manifest.get("tokenizers", []) if item.get("name") == tokenizer_name),
        None,
    )
    ...
    for index in range(val_shards):
        get(f"{dataset_prefix}/fineweb_val_{index:06d}.bin")
    for index in range(train_shards):
        get(f"{dataset_prefix}/fineweb_train_{index:06d}.bin")
